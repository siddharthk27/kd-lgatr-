"""
Evaluate the saved transformer student against the test split.
Mirrors mlp_kd_deepsets/eval_deepsets.py but uses TransformerStudent.
"""
import argparse
import json
import os
...
from mlp_kd_transformer import TransformerStudent             # noqa: E402


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--checkpoint", type=str, default="transformer_student_best.pt")
    ...
    student = TransformerStudent(
        ...
        p_drop=ckpt_args.get("dropout", 0.1),
    ).to(device)
    ...
    print("\n" + "=" * 50)
    print(" KD TRANSFORMER STUDENT — TEST RESULTS ")
    print("=" * 50)
    print(f"AUC:                     {auc:.4f}")
    ...
    plt.plot(fpr, tpr, lw=2, color="darkorange",
             label=f"Transformer KD student (AUC = {auc:.4f})")
    plt.plot([0, 1], [0, 1], lw=2, color="navy", linestyle="--", label="Random")
    plt.xlim([0, 1]); plt.ylim([0, 1.02])
    plt.xlabel("False Positive Rate"); plt.ylabel("True Positive Rate")
    plt.title("ROC — Transformer KD Student")
    plt.legend(loc="lower right"); plt.grid(alpha=0.3)
    plt.savefig(out_dir / "roc_curve_transformer.png", dpi=200, bbox_inches="tight")
    plt.close()
    ...
    plt.ylabel("Density")
    plt.title("Score distribution — Transformer KD Student")
    plt.legend(loc="upper center"); plt.grid(alpha=0.3)
    plt.savefig(out_dir / "prob_dist_transformer.png", dpi=200, bbox_inches="tight")
    plt.close()
    print(f"[eval] wrote {out_dir / 'roc_curve_transformer.png'}")
    print(f"[eval] wrote {out_dir / 'prob_dist_transformer.png'}")
